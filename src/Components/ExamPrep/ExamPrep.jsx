...
const ExamPrep = () => {
  ...
  // Generate vocabulary list for learning
  const generateVocabularyList = async () => {
    setError("");
    setLoading(true);
    ...
    try {
      ...
      if (response.data.success) {
        // Extract words from questions to create a vocabulary list
        const allQuestions = [
          ...(response.data.data.mcq || []),
          ...(response.data.data.trueFalse || []),
          ...(response.data.data.short || [])
        ];
        
        // Create a list of words with definitions (simplified example)
        const extractedWords = allQuestions.map((q, index) => ({
          id: index,
          word: q.question.split(' ')[0], // Simplified extraction
          definition: q.correct || q.answer || "Definition not available",
          example: q.question
        }));
        ...
        setError("Failed to generate vocabulary list: " + response.data.error);
        setLoading(false);
      }
    } catch (err) {
      console.error(err);
  ...
  const nextWord = () => {
    if (currentWordIndex < wordList.length - 1) setCurrentWordIndex(currentWordIndex + 1);
  };
  ...
  const renderLearningMode = () => {
    if (wordList.length === 0) {
      return (
        <div className="bg-white rounded-lg shadow-md p-6 border border-gray-200">
          <h2 className="text-2xl font-bold text-center text-gray-800 mb-6">Vocabulary Learning Mode</h2>
          <p className="text-gray-600 text-center mb-6">
            Learn new words before taking the exam. Select a subject and difficulty level to generate a vocabulary list.
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <div className="flex flex-col justify-end">
              <button 
                onClick={generateVocabularyList} 
                disabled={loading || !subject || !difficulty}
                className="w-full bg-green-600 hover:bg-green-700 text-white font-semibold py-3 px-4 rounded-lg transition-all duration-300 flex items-center justify-center disabled:opacity-70"
              >
                {loading ? (
                  <>
    ...
    return (
      <div className="bg-white rounded-lg shadow-md p-6 border border-gray-200">
        <h2 className="text-2xl font-bold text-center text-gray-800 mb-6">Vocabulary Learning</h2>
        
        <div className="bg-blue-50 rounded-xl p-8 mb-6 border border-blue-200 text-center">
          <h3 className="text-3xl font-bold text-blue-800 mb-4">{currentWord.word}</h3>
          <p className="text-gray-700 text-lg mb-4">{currentWord.definition}</p>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <button 
            ...
            className="bg-gray-600 hover:bg-gray-700 text-white font-semibold py-3 px-6 rounded-lg transition-colors flex-1"
          >
            Back to Selection
          </button>
      ...
      <header className="bg-white shadow-sm border-b">
        <div className="container mx-auto px-4 py-4">
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-2xl font-bold text-blue-800">VocabMaster</h1>
              <p className="text-gray-600">Vocabulary Learning and Exam Platform</p>
            </div>
            <div className="flex gap-2">
              <button 
                onClick={() => setLearningMode(true)}
                className={`px-4 py-2 rounded-lg font-semibold ${learningMode ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-800'}`}
              >
                Learn
              </button>
              <button 
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1">
            {/* Learning Mode */}
            {learningMode && renderLearningMode()}
            ...
            {!learningMode && (
              <>
                ...
                {!quizStarted && !questions && (
                  <div className="bg-white rounded-lg shadow-md p-6 border border-gray-200">
                    <h2 className="text-2xl font-bold text-center text-gray-800 mb-2">Create Your Vocabulary Exam</h2>
                    <p className="text-gray-600 text-center mb-6">Test your vocabulary knowledge with AI-generated questions</p>
